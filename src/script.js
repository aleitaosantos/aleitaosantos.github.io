...
function cuboctaOpen() {
  if (currentIntersect) {
    if (!isCuboctaOpen && !isBoxOpen) {
      ...
      for (let i = 0; i < cuboctaArr.length; i++) {
        gsap.to(cuboctaArr[i].position, {
          x: helpersArr[i].position.x,
          duration: 5,
          ease: 'power3',
        });
        gsap.to(cuboctaArr[i].position, {
          y: helpersArr[i].position.y,
          duration: 5,
          ease: 'power3',
        });
        gsap.to(cuboctaArr[i].position, {
          z: helpersArr[i].position.z,
          duration: 5,
          ease: 'power3',
        });
      }
...
for (let i = 1; i < 7; i++) {
  labels['div' + i].addEventListener('click', () => {
    ...
    gsap.fromTo(
      ...
      {
        boxShadow:
          'rgba(204, 255, 255, 0.4) 0px 0px 20px 0px, rgba(204, 255, 255, 0.6) 0px 0px 1px 1px',
      },
      {
        boxShadow:
          'rgba(102, 102, 153, 0.1) 0px 5px 10px 0px, rgba(102, 102, 153, 0.1) 0px 0px 0px 1px',
        duration: 1.5,
        ease: 'power2.in',
      }
    );
    ...
    if (loaded && !movementStart) {
      ...
      pulseInterval = setInterval(() => {
        if (pulse) {
          ...
          for (let i = 0; i < cuboctaArr.length; i++) {
            pulseAnim['x' + i] = gsap.to(cuboctaArr[i].position, {
              x: helpersArr[i].position.x,
              delay: i * 0.05,
              duration: 1,
              ease: 'sine.inOut',
            });
            pulseAnim['y' + i] = gsap.to(cuboctaArr[i].position, {
              y: helpersArr[i].position.y,
              delay: i * 0.05,
              duration: 1,
              ease: 'sine.inOut',
          ...
          setTimeout(() => {
            if (pulse) {
              ...
              for (let i = 0; i < cuboctaArr.length; i++) {
                ...
                pulseAnim['z' + i] = gsap.to(cuboctaArr[i].position, {
                  z: helpersArr[i].position.z,
                  delay: i * 0.05,
                  duration: 1,
                  ease: 'sine.inOut',
